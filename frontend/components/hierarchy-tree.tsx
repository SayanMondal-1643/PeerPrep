...
export function HierarchyTree({
  ...
  const handleNameModalSubmit = (name: string) => {
    ...
    if (nameModal.type === "addExam") {
      ...
      const exam = newExams.find((e: Exam) => e.id === nameModal.examId);
      const branch = exam?.branches.find(
        (b: Branch) => b.id === nameModal.branchId,
      );
      if (branch) {
      ...
      const subject = branch?.subjects.find(
        (s: Subject) => s.id === nameModal.subjectId,
      );
      if (subject) {
        ...
        exam.topicCount += 1;
        setExpandedSubjects(
          new Set([...expandedSubjects, nameModal.subjectId]),
        );
      ...
      const topic = subject?.topics.find(
        (t: Topic) => t.id === nameModal.topicId,
      );
      if (topic) {
        topic.name = name;
  ...
  const handleDeleteConfirm = () => {
    ...
    if (deleteModal.type === "exam" && deleteModal.examId) {
      const index = newExams.findIndex(
        (e: Exam) => e.id === deleteModal.examId,
      );
      if (index > -1) {
      ...
      if (exam) {
        const branch = exam.branches.find(
          (b: Branch) => b.id === deleteModal.branchId,
        );
        const index = exam.branches.findIndex(
          (b: Branch) => b.id === deleteModal.branchId,
        );
        if (index > -1) {
          exam.branchCount -= 1;
      ...
      if (branch) {
        const subject = branch.subjects.find(
          (s: Subject) => s.id === deleteModal.subjectId,
        );
        const index = branch.subjects.findIndex(
          (s: Subject) => s.id === deleteModal.subjectId,
        );
        if (index > -1) {
          branch.subjectCount -= 1;
      ...
      if (subject) {
        const topic = subject.topics.find(
          (t: Topic) => t.id === deleteModal.topicId,
        );
              ...
              <div className="flex items-center gap-3 flex-1 min-w-0">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-sm text-muted-foreground">
                    {exam.branchCount} Branches • {exam.subjectCount} Subjects •{" "}
                    {exam.topicCount} Topics • {exam.materialCount} Materials
                  </p>
                </div>
              </div>
                  ...
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <Button
                        size="sm"
                        variant="ghost"
                        onClick={() => handleAddBranch(exam.id, exam.name)}
                      >
                      ...
                      <Button
                        ...
                        onClick={() =>
                          handleDeleteExam(exam.id, exam.name, {
                            branches: exam.branchCount,
                            subjects: exam.subjectCount,
                            topics: exam.topicCount,
                            materials: exam.materialCount,
                          })
                        }
                      >
                    ...
                    >
                      <div className="flex items-center gap-3 flex-1 min-w-0">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-sm text-muted-foreground">
                            {branch.subjectCount} Subjects • {branch.topicCount}{" "}
                            Topics • {branch.materialCount} Materials
                          </p>
                        </div>
                      </div>
                          ...
                          <Tooltip>
                            <TooltipTrigger asChild>
                              <Button
                                ...
                                variant="ghost"
                                onClick={() =>
                                  handleEditBranch(branch, exam.id)
                                }
                              >
                                <Edit className="h-4 w-4" />
                              </Button>
                              ...
                              <Button
                                ...
                                onClick={() =>
                                  handleDeleteBranch(
                                    ...
                                    branch.name,
                                    {
                                      subjects: branch.subjectCount,
                                      topics: branch.topicCount,
                                      materials: branch.materialCount,
                                    },
                                  )
                                }
                              >
                                        ...
                                        onClick={() =>
                                          handleDeleteSubject(
                                            ...
                                            subject.name,
                                            {
                                              ...
                                              materials: subject.materialCount,
                                            },
                                          )
                                        }
                                      >
